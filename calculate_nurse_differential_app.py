    ...
    for start_time, end_time, is_on_call in work_periods:
        ...
        while current_time < end_time:
            ...
            if charge_nurse_pay > 0:
                ...
                rate *= (1 + night_percent / 100)
            if weekend:
                rate *= (1 + weekend_percent / 100)
            if is_on_call:
                rate *= (1 + on_call_percent / 100)
...
if st.button("Add Work Period"):
    ...
    if check_overlap(start_datetime, end_datetime, st.session_state.work_periods):
        ...
        formatted_shift = f"{date.strftime('%B %d')}, {shift_type} from {start_time.strftime('%I:%M %p').lower()} to {end_time.strftime('%I:%M %p').lower()}"
...
if st.session_state.work_periods:
    ...
    for i, (start, end, is_on_call) in enumerate(st.session_state.work_periods):
        ...
        formatted_shift = f"{start.strftime('%B %d')}, {shift_type} from {start.strftime('%I:%M %p').lower()} to {end.strftime('%I:%M %p').lower()}"
...
night_percent = st.number_input("Night Shift Differential (%)", min_value=0.0, value=16.0)
weekend_percent = st.number_input("Weekend Differential (%)", min_value=0.0, value=5.0)

# Calculate earnings
if st.button("Calculate Earnings"):
    total_earnings, total_hours = calculate_weekly_earnings(
        st.session_state.work_periods, hourly_rate, charge_nurse_pay, night_percent, weekend_percent, on_call_percent
    )
